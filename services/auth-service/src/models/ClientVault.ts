...
const ClientVaultSchema = new Schema<IClientVault>(
  ...
  { timestamps: false, collection: 'client_vault' }
);

// Retrieve ClientVault model on secureDb
export const getClientVaultModel = (): mongoose.Model<IClientVault> => {
  if (secureDb) return secureDb.models.ClientVault || secureDb.model<IClientVault>('ClientVault', ClientVaultSchema);
