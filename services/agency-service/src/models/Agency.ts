...
import { operationsDb } from '../config/database.js';
...
// Retrieve Client model on operationsDb
export const getClientModel = (): mongoose.Model<IClient> => {
  if (operationsDb) return operationsDb.models.Client || operationsDb.model<IClient>('Client', ClientSchema);
  return mongoose.models.Client || mongoose.model<IClient>('Client', ClientSchema);
};

// Retrieve Project model on operationsDb
export const getProjectModel = (): mongoose.Model<IProject> => {
  if (operationsDb) return operationsDb.models.Project || operationsDb.model<IProject>('Project', ProjectSchema);
  return mongoose.models.Project || mongoose.model<IProject>('Project', ProjectSchema);
};

// Retrieve Task model on operationsDb
export const getTaskModel = (): mongoose.Model<ITask> => {
  if (operationsDb) return operationsDb.models.Task || operationsDb.model<ITask>('Task', TaskSchema);
  return mongoose.models.Task || mongoose.model<ITask>('Task', TaskSchema);
};

// Retrieve Team model on operationsDb
export const getTeamModel = (): mongoose.Model<ITeam> => {
  if (operationsDb) return operationsDb.models.Team || operationsDb.model<ITeam>('Team', TeamSchema);
  return mongoose.models.Team || mongoose.model<ITeam>('Team', TeamSchema);
};

// Retrieve Employee model on operationsDb
export const getEmployeeModel = (): mongoose.Model<IEmployee> => {
  if (operationsDb) return operationsDb.models.Employee || operationsDb.model<IEmployee>('Employee', EmployeeSchema);
