...
// Retrieve Invoice model on secureDb
export const getInvoiceModel = (): mongoose.Model<IInvoice> => {
  if (secureDb) return secureDb.models.Invoice || secureDb.model<IInvoice>('Invoice', InvoiceSchema);
  return mongoose.models.Invoice || mongoose.model<IInvoice>('Invoice', InvoiceSchema);
};

// Retrieve Payment model on secureDb
export const getPaymentModel = (): mongoose.Model<IPayment> => {
  if (secureDb) return secureDb.models.Payment || secureDb.model<IPayment>('Payment', PaymentSchema);
  return mongoose.models.Payment || mongoose.model<IPayment>('Payment', PaymentSchema);
};

// Retrieve Bill model on secureDb
export const getBillModel = (): mongoose.Model<IBill> => {
  if (secureDb) return secureDb.models.Bill || secureDb.model<IBill>('Bill', BillSchema);
  return mongoose.models.Bill || mongoose.model<IBill>('Bill', BillSchema);
};

// Retrieve Payroll model on secureDb
export const getPayrollModel = (): mongoose.Model<IPayroll> => {
  if (secureDb) return secureDb.models.Payroll || secureDb.model<IPayroll>('Payroll', PayrollSchema);
