...
// Retrieve SupportTicket model on operationsDb
export const getSupportTicketModel = (): mongoose.Model<ISupportTicket> => {
  if (operationsDb) return operationsDb.models.SupportTicket || operationsDb.model<ISupportTicket>('SupportTicket', SupportTicketSchema);
  return mongoose.models.SupportTicket || mongoose.model<ISupportTicket>('SupportTicket', SupportTicketSchema);
};

// Retrieve ProjectDoc model on operationsDb
export const getProjectDocModel = (): mongoose.Model<IProjectDoc> => {
  if (operationsDb) return operationsDb.models.ProjectDoc || operationsDb.model<IProjectDoc>('ProjectDoc', ProjectDocSchema);
  return mongoose.models.ProjectDoc || mongoose.model<IProjectDoc>('ProjectDoc', ProjectDocSchema);
};

// Retrieve TelemetryLog model on operationsDb
export const getTelemetryLogModel = (): mongoose.Model<ITelemetryLog> => {
  if (operationsDb) return operationsDb.models.TelemetryLog || operationsDb.model<ITelemetryLog>('TelemetryLog', TelemetryLogSchema);
